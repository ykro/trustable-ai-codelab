...
${instruction}`;
  ...
  private runFeedforward(frame: TelemetryFrame) {
    ...
    // fall back to nearest-only inside findNearestCorner.
    ...
    const nearest = this.findNearestCorner(
      frame.latitude, frame.longitude, this.track.corners, heading,
    );
  ...
  /**
   * Pick the geometrically closest corner within 150m, with a heading-aware
   * "ahead of the driver" filter when heading is known. At Sonoma's T1/T2/T3
   * complex the geofences cluster along a single approach line, and a pure
   * nearest-wins picker latches onto the next corner only after the driver
   * crosses the C1↔C2 midpoint — collapsing time-to-corner well below DR-1's
   * 3.0s budget. Rejecting corners ≥ 90° behind the driver fixes that.
   *
   * Fallback: when `heading` is null (no GPS history, stationary, or sub-0.5m
   * movement), we use the original nearest-only behavior so fresh sessions
   * still fire FEEDFORWARD on the first frame after track load.
   */
  private findNearestCorner(
    lat: number, lon: number, corners: Corner[], heading: number | null,
  ): Corner | null {
